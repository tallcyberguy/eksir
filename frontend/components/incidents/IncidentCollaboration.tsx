...
export function IncidentCollaboration({ incidentId }: { incidentId: string }) {
  ...
  const load = useCallback(() => {
    api.incidentComments(incidentId).then(setComments).catch(() => {});
    api.incidentWatchers(incidentId).then(setWatchers).catch(() => {});
  }, [incidentId]);

  useEffect(() => {
    api.me().then((u) => setMeId(u?.id ?? null)).catch(() => {});
    api.incidentMentionableUsers(incidentId).then(setUsers).catch(() => {});
    load();
  }, [incidentId, load]);
  ...
  async function toggleWatch() {
    try {
      if (amWatching && meId) await api.removeIncidentWatcher(incidentId, meId);
      else await api.addIncidentWatcher(incidentId);
      api.incidentWatchers(incidentId).then(setWatchers).catch(() => {});
    } catch {
      /* non-fatal */
    }
  ...
  async function submit() {
    ...
    setBusy(true);
    try {
      await api.addIncidentComment(incidentId, text);
      setBody("");
